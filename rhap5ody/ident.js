class Ident {
  ...
  setTreeId(id) {
    ...
    if (
      id.length > 0 &&
      id[0] == "@" &&
      (id.length == 1 || regex.test(id.substring(1)))
    ) {
      this.treeid = id;
  ...
  _searchForTreeId(id) {
    ...
    for (var p of path) {
      ...
      if (next != undefined) o = next;
        ...
        return { found: false, object: null, err: "not found in object tree" };
    }
  ...
  _searchForNameId(nameid) {
    ...
    if (this.getNameId() == nameid)
      return { found: true, object: this.obj, err: null };
    // else check if root
    if (this.getTreeId() == "@") {
  ...
  _searchForClassName(classname) {
    ...
    if (results.length > 0) return { found: true, object: results, err: null };
      ...
      return { found: false, object: this.obj, err: "no objects with class" };
  }
